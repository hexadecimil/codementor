...
// Chemin absolu vers les fichiers de routes ; on normalise les antislashs Windows
// en slashes, car le glob de swagger-jsdoc ne reconnaît que les slashes.
const routesGlob = path.join(path.dirname(fileURLToPath(import.meta.url)), "routes", "*.js").replace(/\\/g, "/");
...
2. Le serveur stocke la session après le callback GitHub.
    ...
    servers: [{ url: "http://localhost:3000/codementor/api", description: "Serveur local" }],
    components: {
      securitySchemes: {
        sessionAuth: {
          type: "apiKey",
      ...
      schemas: {
        ...
        AnalysisSummary: {
          type: "object",
          description: "Version allégée renvoyée dans l'historique d'un projet (liste).",
          properties: {
            id: { type: "integer", example: 1 },
            status: {
              type: "string",
