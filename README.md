...
Le projet a été développé de février à juin 2026 sur le GitLab de l'École des Métiers de Fribourg,
puis publié ici une fois le travail terminé. Le rapport de TPI (analyse, conception, tests) n'est
pas inclus dans le dépôt. Ce README explique comment installer, configurer et utiliser
l'application.

## Points techniques

- **Authentification par OAuth GitHub** : aucun mot de passe demandé ni stocké ; le token d'accès
  est chiffré en base en AES-256-GCM avant persistance.
- **File d'analyse asynchrone** : les analyses s'exécutent en arrière-plan avec une concurrence
  configurable (par analyse et par fichier), l'interface suit la progression en temps réel.
- **API REST documentée** : chaque endpoint est décrit et testable dans Swagger, avec paramètres,
  codes de réponse et exemples.
- **Coût LLM maîtrisé** : modèle et mode de raisonnement configurables, ce qui permet d'arbitrer
  entre précision des détections et coût des tokens.
...
l'API REST (backend) consommée par cette interface.
...
Le périmètre du TPI étant volontairement borné, plusieurs évolutions sont identifiées :

- **Parallélisation par fenêtre glissante.** Les fichiers sont aujourd'hui analysés par lots de
  `FILE_CONCURRENCY` (`Promise.all` par lot) : chaque lot attend son fichier le plus lent avant de
  lancer le suivant. Un pool de workers maintenant en permanence N appels en cours réduirait
  nettement la durée totale d'une analyse.
- Analyse incrémentale : ne réanalyser que les fichiers modifiés depuis la dernière analyse, pour
  réduire le coût et la durée.
- Tests automatisés (unitaires sur les services, end-to-end sur le parcours d'analyse).
- Score de qualité global par analyse, pour suivre l'évolution d'un dépôt dans le temps.
- Support d'autres forges (GitLab, Bitbucket) en plus de GitHub.
- Durcissement du prompt pour améliorer encore la précision des détections.
